...
<p align="center">
  <img src="assets/capturas/repositorio_frontend.png" alt="commits grafico" width="700">
</p>
<p align="center">
  <img src="assets/capturas/commits_frontend.png" alt="commits grafico" width="700">
</p>

- URL del repositorio del backend de "StayMap": `https://github.com/The-Rumbling-AW/StayMap-WebService.git`

<p align="center">
  <img src="assets/capturas/repositorio_webservice.png" alt="commits grafico" width="700">
</p>
<p align="center">
  <img src="assets/capturas/commits_webservice.png" alt="commits grafico" width="700">
</p>
...
| TS-21 | Obtener concierto por ID | Como developer, quiero un endpoint GET en /api/v1/concerts/{id} que devuelva los datos de un concierto específico. | **Escenario: Consulta de evento existente**<br>Dado que el ID es válido<br>Cuando se llama al endpoint<br>Entonces se devuelven los datos del concierto | EP01 / EP06 |
...
| TS-25 | Buscar conciertos por plataforma | Como developer, quiero un endpoint GET en /api/v1/concerts/by-platform/{platform} que devuelva conciertos según su plataforma (Joinnus, Teleticket, etc.). | **Escenario: Filtro por plataforma**<br>Dado que se especifica una plataforma<br>Cuando se hace GET<br>Entonces se devuelven solo los conciertos que la usan | EP01 / EP06 |
| TS-26 | Obtener usuario por ID | Como developer, quiero un endpoint GET en /api/v1/users/{id} para ver el perfil público de un usuario. | **Escenario: Acceso a perfil**<br>Dado que el ID es válido<br>Cuando se consulta<br>Entonces devuelve nombre, imagen y comunidades visibles | EP04 / EP06 |
| TS-27 | Actualizar perfil de usuario | Como developer, quiero un endpoint PUT en /api/v1/users/{id} para permitir al usuario modificar su información personal. | **Escenario: Edición correcta**<br>Dado que el usuario envía un body válido<br>Cuando se hace PUT<br>Entonces se actualiza su perfil | EP04 / EP06 |
...
| TS-29 | Dar like a publicación | Como developer, quiero un endpoint POST en /api/v1/users/{userId}/like que registre un like del usuario a un post. | **Escenario: Like aplicado**<br>Dado que el usuario quiere reaccionar<br>Cuando hace POST<br>Entonces se incrementa el contador de likes | EP04 / EP06 |
| TS-30 | Quitar like a publicación | Como developer, quiero un endpoint DELETE en /api/v1/users/{userId}/like para eliminar el like de un usuario a una publicación. | **Escenario: Like eliminado**<br>Dado que ya reaccionó<br>Cuando hace DELETE<br>Entonces se reduce el contador de likes | EP04 / EP06 |
...
| TS-32 | Ver publicación por ID | Como developer, quiero un endpoint GET en /api/v1/post/{id} para obtener el detalle de una publicación. | **Escenario: Consulta de publicación**<br>Dado que el ID es válido<br>Cuando se hace GET<br>Entonces se devuelven los detalles | EP04 / EP06 |
| TS-33 | Eliminar publicación por ID | Como developer, quiero un endpoint DELETE en /api/v1/post/{id} para permitir que un usuario borre su publicación. | **Escenario: Eliminación exitosa**<br>Dado que es el autor<br>Cuando hace DELETE<br>Entonces el post se elimina | EP04 / EP06 |
| TS-34 | Ver publicaciones por comunidad | Como developer, quiero un endpoint GET en /api/v1/post/by-community/{communityId} que liste todas las publicaciones de una comunidad. | **Escenario: Listado por comunidad**<br>Dado que el usuario accede a una comunidad<br>Cuando hace GET<br>Entonces se muestran sus publicaciones | EP04 / EP06 |
| TS-35 | Registro de usuario | Como developer, quiero un endpoint POST en /api/v1/authentication/sign-up para registrar nuevos usuarios con contraseña encriptada. | **Escenario: Registro exitoso**<br>Dado que el usuario envía datos válidos<br>Cuando se hace POST<br>Entonces se guarda y se aplica hashing (bcrypt)<br>**Escenario: Email duplicado**<br>Dado que el correo ya existe<br>Cuando se intenta registrar<br>Entonces el servidor responde 409 | EP05 / EP06 |
...
| 52      | TS-14          | Endpoint para obtener conciertos        | Como developer, quiero permitir listar todos los conciertos públicos disponibles.                        | 2                        |
| 53      | TS-15          | Endpoint para confirmar asistencia      | Como developer, quiero permitir que un usuario confirme su asistencia a un evento.                       | 3                        |
| 54      | TS-16          | Endpoint para cancelar asistencia       | Como developer, quiero permitir que un usuario cancele su asistencia a un evento.                        | 2                        |
| 55      | TS-17          | Endpoint para obtener comunidades       | Como developer, quiero permitir listar todas las comunidades disponibles.                                | 1                        |
| 56      | TS-18          | Endpoint para unirse a comunidad        | Como developer, quiero permitir que un usuario se una a una comunidad.                                   | 2                        |
| 57      | TS-19          | Endpoint para salir de comunidad        | Como developer, quiero permitir que un usuario abandone una comunidad.                                   | 2                        |
| 58      | TS-20          | Endpoint para crear concierto           | Como developer, quiero permitir registrar un nuevo concierto en la plataforma.                           | 3                        |
| 59      | TS-21          | Endpoint para ver concierto por ID      | Como developer, quiero permitir consultar los detalles de un concierto específico.                       | 1                        |
| 60      | TS-22          | Endpoint para eliminar concierto        | Como developer, quiero permitir eliminar un concierto registrado.                                        | 2                        |
| 61      | TS-23          | Endpoint para alternar asistencia       | Como developer, quiero permitir alternar el estado de asistencia del usuario a un concierto.             | 3                        |
| 62      | TS-24          | Endpoint para actualizar estado         | Como developer, quiero permitir modificar el estado actual de un concierto (activo, cancelado, etc.).    | 3                        |
| 63      | TS-25          | Endpoint para buscar por plataforma     | Como developer, quiero permitir filtrar conciertos según la plataforma (Joinnus, Teleticket, etc.).      | 2                        |
| 64      | TS-26          | Endpoint para ver perfil de usuario     | Como developer, quiero permitir consultar el perfil público de un usuario.                               | 1                        |
| 65      | TS-27          | Endpoint para actualizar perfil         | Como developer, quiero permitir que un usuario edite su perfil.                                          | 2                        |
| 66      | TS-28          | Endpoint para listar usuarios           | Como developer, quiero permitir listar todos los usuarios registrados.                                   | 1                        |
| 67      | TS-29          | Endpoint para dar like                  | Como developer, quiero permitir que un usuario reaccione con un like a una publicación.                  | 2                        |
| 68      | TS-30          | Endpoint para quitar like               | Como developer, quiero permitir que un usuario elimine su like de una publicación.                       | 2                        |
| 69      | TS-31          | Endpoint para crear publicación         | Como developer, quiero permitir que un usuario publique texto o imagen en una comunidad.                 | 3                        |
| 70      | TS-32          | Endpoint para ver publicación por ID    | Como developer, quiero permitir consultar el contenido de una publicación específica.                    | 1                        |
| 71      | TS-33          | Endpoint para eliminar publicación      | Como developer, quiero permitir que un usuario elimine una publicación propia.                           | 2                        |
| 72      | TS-34          | Endpoint para ver publicaciones por comunidad | Como developer, quiero permitir ver todas las publicaciones asociadas a una comunidad.              | 2                        |
| 73      | TS-35          | Endpoint para registro de usuario       | Como developer, quiero permitir que un nuevo usuario se registre con seguridad.                          | 3                        |
| 74      | TS-36          | Endpoint para login de usuario          | Como developer, quiero permitir que un usuario inicie sesión con validación segura.                      | 3                        |
...
- TF:  `https://upcedupe-my.sharepoint.com/:v:/g/personal/u202311558_upc_edu_pe/EfKLH8uULHJHlO8RrzS4Jh0B5-SB35o3JA0cBLpJZTh0Sw?e=9MeAsv&nav=eyJyZWZlcnJhbEluZm8iOnsicmVmZXJyYWxBcHAiOiJTdHJlYW1XZWJBcHAiLCJyZWZlcnJhbFZpZXciOiJTaGFyZURpYWxvZy1MaW5rIiwicmVmZXJyYWxBcHBQbGF0Zm9ybSI6IldlYiIsInJlZmVycmFsTW9kZSI6InZpZXcifX0%3D`
